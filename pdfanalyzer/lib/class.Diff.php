...
class Diff{
  ...
  public static function compare(
    ...
    if ($compareCharacters){
      $sequence1 = $string1;
      $sequence2 = $string2;
      $end1 = mb_strlen($string1, 'UTF-8') - 1;
      $end2 = mb_strlen($string2, 'UTF-8') - 1;
    }else if (is_string($string1) && is_string($string2)){
      $sequence1 = preg_split('/\R/u', $string1);
      $sequence2 = preg_split('/\R/u', $string2);
      $end1 = count($sequence1) - 1;
      $end2 = count($sequence2) - 1;
    }else if (is_array($string1) && is_array($string2)) {
      $sequence1 = $string1;
      $sequence2 = $string2;
      $end1 = count($sequence1) - 1;
      $end2 = count($sequence2) - 1;
    ...
    for ($index = $end1 + 1;
         $index < ($compareCharacters ? mb_strlen($sequence1, 'UTF-8') : count($sequence1));
        $index ++){
      $diff[] = array($sequence1[$index], self::UNMODIFIED);
    }
